...
y,e,Y,z,v,eps,Eps,S,sigma,sigma_E = [None]*10
#interpolate = utilities.interpolator_factory([3])

def calibrate(Para):
    global F,G,f,ny,ne,nY,nz,nv,n,Ivy,Izy,nG
    global y,e,Y,z,v,eps,S,sigma,Eps,sigma_E
    #global interpolate
    F,G,f = Para.F,Para.G,Para.f
    ny,ne,nY,nz,nv,n,nG = Para.ny,Para.ne,Para.nY,Para.nz,Para.nv,Para.n,Para.nG
    ...
    Eps = np.arange(ny+ne+nY+nz+nv+1,ny+ne+nY+nz+nv+2).view(hashable_array)
...
class approximate(object):
    ...
    def get_wf(self,z_i):
        ...
        return np.hstack((
        ybar,ebar,Ybar,ybar[:nz],Ivy.dot(ybar),0.,0.
        ))
    ...
    def iterate(self,quadratic = True):
        ...
        y,epsilon = ye[:,:-1],ye[:,-1]
        Gamma = y.dot(Izy.T)
        Y = self.ss.get_Y() + self.dY_Eps.flatten() * E + 0.5*quadratic*self.d2Y[sigma].flatten()*sigma**2
