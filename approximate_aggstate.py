...
class approximate(object):
    ...
    def get_df(self,z_i):
        ...
        d[y,S],d[y,eps],d[y,Z] = np.hstack((dy[z](z_i),dy[Y](z_i))),dy[eps](z_i), dy[Z](z_i) # first order effect of S and eps on y
    
        d[e,S],d[e,eps],d[e,Z] = df.dot(d[y,S]), np.zeros((ne,1)),df.dot(d[y,Z]) # first order effect of S on e
    
        d[Y,S],d[Y,eps],d[Y,Z] = np.hstack(( np.zeros((nY,nz)), np.eye(nY) )),np.zeros((nY,neps)),self.dY_Z
