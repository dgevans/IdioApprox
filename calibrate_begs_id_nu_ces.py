...
ny = 13 # number of individual controls (m_{t},mu_{t},c_{t},l_{t},rho1_,rho2,phi,x_{t-1},kappa_{t-1}) Note that the forward looking terms are at the end
...
def F(w):
    ...
    '''
    logm,muhat,e,c,l,rho1_,rho2,phi,w_e,UcP,a,x_,kappa_ = w[:ny] #y
    ...
    ret[9] = e - nu*e_ - eps_p
    ret[10] = w_e - np.exp(e+eps_t+Eps)
    ret[11] = UcP - Uc*P
    ret[12] = a - x/Uc
    
    ret[13] = mu_*EUcP - EUc_muP
    ret[14] = alpha1 - m_*EUcP # bond pricing
...
def f(y):
    ...
    '''
    logm,muhat,e,c,l,rho1_,rho2,phi,w_e,UcP,a,x_,kappa_ = y
...
def Finv(YSS,z):
    ...
    return np.vstack((
    logm,muhat,e,c,l,rho1,rho2,phi,w_e,Uc,a,x,kappa   
    ))
...
def GSS(YSS,y_i):
    ...
    '''
    logm,muhat,e,c,l,rho1,rho2,phi,w_e,UcP,a,x,kappa = y_i
    alpha1,alpha2,tau,eta,lamb,T = YSS
