...
def F(w):
    ...
    Eps = w[ny+ne+nY+nz+nv+1] #aggregate shock
    ...
    ret[4] = (1-tau)*np.exp(e+Eps)*Uc + Ul #wage
    ret[5] = Ul - mu*(Ull*l + Ul) - phi*Ull + lamb*np.exp(Eps+e)
    ret[6] = rho2 + kappa/Uc + eta/Uc
    ret[7] = Uc + x_*Ucc/(beta*EUc)*(mu-mu_) - mu*(Ucc*c + Uc) + rho1_*m_*Ucc/beta \
             +rho2*m*Ucc - phi*np.exp(e+Eps)*(1-tau)*Ucc - lamb
...
def G(w):
    ...
    ret[3] = c - np.exp(e+Eps) * l # resources
